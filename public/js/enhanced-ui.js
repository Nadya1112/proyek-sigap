...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document
        .querySelectorAll(
            ".feature-card, .benefit-card, .animate-fade-in, .animate-slide-up"
        )
        .forEach((el) => {
            el.style.opacity = "0";
    ...
    document
        .querySelectorAll(".feature-card, .benefit-card")
        .forEach((card) => {
    ...
    document
        .querySelectorAll(
            "button, .feature-card-btn, .cta-btn-primary, .cta-btn-secondary"
        )
        .forEach((button) => {
    ...
    document
        .querySelectorAll(
            ".feature-card-btn, .cta-btn-primary, .cta-btn-secondary"
        )
        .forEach((btn) => {
    ...
    document
        .querySelectorAll(".feature-icon, .benefit-icon")
        .forEach((icon) => {
    ...
    const heroSubtitle = document.querySelector(
        'header p[style*="animation-delay: 0.6s"]'
    );
    if (heroSubtitle) {
    ...
    const enhancedStatsObserver = new IntersectionObserver(
        (entries) => {
            entries.forEach((entry) => {
                if (entry.isIntersecting) {
                    const counters = entry.target.querySelectorAll(".counter");
    ...
    document.addEventListener("keydown", (e) => {
        ...
        if (konamiCode === konamiSequence) {
            ...
            document.body.style.background =
                "linear-gradient(45deg, #F59E0B, #F97316, #EF4444, #EC4899)";
            document.body.style.backgroundSize = "400% 400%";
            document.body.style.animation = "gradient 2s ease infinite";

            showToast(
                "🎉 Easter Egg Activated! SIGAP KOMPLEK Special Mode!",
                "success"
            );
    ...
    if ("performance" in window) {
        window.addEventListener("load", () => {
            ...
            if (loadTime > 3000) {
                console.warn(
                    "⚠️ Slow loading detected. Consider optimizing images and assets."
                );
            }
        });
    }
    ...
    window.addEventListener("error", (e) => {
        ...
        document
            .querySelectorAll(".animate-float, .animate-bounce-in")
            .forEach((el) => {
                el.style.animation = "none";
            });
    });
